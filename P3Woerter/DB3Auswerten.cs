...
namespace Borys.Nachrichten
{
  ...
  {
    private static void W3Statistik(string connectionString, uint länge, string dBTabelle)
    {
      double rfToken;
    ...
    //private static void DB3Auswerten(string connectionString, string dBTMeldungen, string dBTWoerter, string kennung)
    //{
    ...
    //      if (rMeld.HasRows)
    //      {
    //        (neuWort, nMld, nToken) = WörterAusMeldungen(dBTMeldungen, dBTWoerter, kennung, 3, SQLWrt, rMeld);
    //      }
    //      else
    //      {
    ...
    private static (int, int) W3ausMld(MySqlCommand SQL, string dBTWoerter, string[] WörterEinerMeldung)
    {
      int neuWort = 0;
      int nToken = 0;
      string wort2 = null, wort3 = null;
      try
      {
        foreach (string wort in WörterEinerMeldung)
        {
          ...
          wortkette = $"{wort1} {wort2} {wort3}";
          nToken++;
          //Console.WriteLine(wort);
          SQL.CommandText = $"SELECT anzahl FROM {dBTWoerter} WHERE (wort = '{wortkette}')";
          rWort = SQL.ExecuteReader();//da w2 Schlüssel ist kann es nur einen Record geben
          if (rWort.HasRows)
          {
            int nNeu;
            _ = rWort.Read();
      ...
      catch (MySqlException ex)
      {
        ...
        if (ex.Number == 1146)
        {
          ...
          SQL.CommandText = cTextauswertung.CREATE3WOERTER;
          _ = SQL.ExecuteNonQuery();
